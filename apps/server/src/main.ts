...
/**
 * @description: 应用程序的入口文件，负责引导 NestJS 应用程序的启�? * bootstrap 函数是一个异步函数，使用 NestFactory 创建一�?NestJS 应用程序实例，并进行一些基本的配置，如启用 CORS、设置全局前缀、监听指定端口等。它还注册了全局的响应拦截器和异常过滤器，以统一处理 API 响应格式和异常情况。最后，调用 bootstrap 函数来启动应用程�? * @returns void
 */
