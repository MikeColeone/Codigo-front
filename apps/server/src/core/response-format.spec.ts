...
import { ResponseIntercept } from 'src/core/interceptor/response.interceptor';
...
describe('response format', () => {
  ...
  it('maps bad request exception to invalid params code', () => {
    const filter = new AbnormalFilter();
    const response = createResponseMock();
    const exception = new BadRequestException(['phone must not be empty']);
    ...
    expect(payload).toMatchObject({
      ...
      error: {
        code: Code.InvalidParams,
        status: 400,
        details: ['phone must not be empty'],
      },
    });
  });
